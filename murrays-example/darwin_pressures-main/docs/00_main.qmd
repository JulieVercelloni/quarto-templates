...
```{r process spatial glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process spatial subcatchments glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process data 2012_2015 glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process data 2016_2022 glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process data glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process primary glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process erp glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process catchment_erp glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process fire_freq glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process fire_areas glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process sea_level_fin glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process rainfall_fin glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process rainfall_anom glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process temp glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process temp_anom glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process SOI_fin glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process SST_anom glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process build glimpse, results='markdown', eval=PROCESS_TABLES}
...
```
```{r process ship glimpse, results='markdown', eval=PROCESS_TABLES}
...
```{r process data Combine glimpse, results='markdown', eval=PROCESS_TABLES}
...
# Exploratory trees (Routine sampling, no lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```

```{r TreesTable, results='markdown', eval=TRUE}
load(file = '../data/modelled/Tables.RData')
```

::: panel-tabset
```{r Trees Routine, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        ...
        cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ",lab,
                      " for routine samples for the whole harbour. ",
                      "")
        ##if (INCLUDE_FIGURES)
        cat(paste0("![",cap,"](../output/figures/Trees_routine_",j,".png){#fig-Tree-routine-",j,"} \n\n"))
    }
}
```
:::
...
# Exploratory trees (Routine sampling, with lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```
::: panel-tabset
```{r Trees Routine lags, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        lab <- units_lookup %>%
            filter(Measure == j) %>%
        ...
        cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ",lab,
                      " for routine samples for the whole harbour including lags (0-2). ",
                      "")
        ##if (INCLUDE_FIGURES)
        cat(paste0("![",cap,"](../output/figures/Trees_routine_lags_",j,".png){#fig-Tree-routine-lags-",j,"} \n\n"))
    }
}
```
...
# Exploratory trees (Discrete sampling. no lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```
::: panel-tabset
```{r Trees Discrete, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        lab <- units_lookup %>%
            filter(Measure == j) %>%
        ...
        cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ",lab,
                      " for discrete samples for the whole harbour. ",
                      "")
        ##if (INCLUDE_FIGURES)
        cat(paste0("![",cap,"](../output/figures/Trees_discrete_",j,".png){#fig-Tree-discrete-",j,"} \n\n"))
    }
}
```
...
# Exploratory trees (Discrete sampling. with lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```
::: panel-tabset
```{r Trees Discrete lags, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        lab <- units_lookup %>%
            filter(Measure == j) %>%
        ...
        cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ",lab,
                      " for discrete samples for the whole harbour including lags (0-2). ",
                      "")
        ##if (INCLUDE_FIGURES)
        cat(paste0("![",cap,"](../output/figures/Trees_discrete_lags_",j,".png){#fig-Tree-discrete-lags-",j,"} \n\n"))
    }
}
```
...
# Exploratory trees (Routine sampling by Zone, no lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```
::: panel-tabset

```{r Trees Routine Zone, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        lab <- units_lookup %>%
            filter(Measure == j) %>%
        ...
        cat(paste0("\n::::: panel-tabset\n\n"))
        for (k in unique(spatial_lookup$ZoneName)) {
            cat(paste0("\n#### ", k, "\n"))
            
            cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ", lab,
                          " for routine samples within the ", k, "Zone. ",
                          "")
            ##if (INCLUDE_FIGURES)
            cat(paste0("![",cap,"](../output/figures/Trees_routine_",j,"__",k,".png){#fig-Tree-routine-",j,"__",k,"} \n\n"))
        }
...
:::
# Exploratory trees (Discrete sampling by Zone, no lags) 

```{r EDA load lookups, results='markdown', eval=TRUE}
```
::: panel-tabset

```{r Trees Discrete Zone, results='asis', eval=TRUE, echo=FALSE}
if (PERFORM_TREES) {
    for (j in FOCAL_RESPS) {
        lab <- units_lookup %>%
            filter(Measure == j) %>%
        ...
        for (k in unique(spatial_lookup$ZoneName)) {
            ...
            cap <- paste0("Boosted regression tree relative influence (left) and selected partial trends (right) for ", lab,
                          " for discrete samples within the ", k, "Zone. ",
                          "")
            ##if (INCLUDE_FIGURES)
            cat(paste0("![",cap,"](../output/figures/Trees_discrete_",j,"__",k,".png){#fig-Tree-discrete-",j,"__",k,"} \n\n"))
        }
...
```{r 50_summaries getFiles, results='markdown', eval=FALSE}
```

```{r 50_summaries compile tree stats, results='markdown', eval=FALSE}
```
```{r 50_summaries functions, results='markdown', eval=TRUE}
```
```{r 50_summaries corr and R2 tables, results='markdown', eval=FALSE}
```

```{r 50_summaries format tables, results='markdown', eval=FALSE}
```
</details>


```{r name2, results='asis', eval=TRUE}
load(file = paste0(DATA_PATH, "modelled/formatted.tables.RData"))
  
cat("::: {.panel-tabset}\n\n")
walk(unique(formatted.table$Type), ~ {
    Type = .x
    cat(paste0("## ", .x, "\n\n"))
    cat(":::: {.panel-tabset}\n\n")
    tab.type <- formatted.table %>% filter(Type == .x)
    walk(c("Whole Harbour", "Zones"), ~ {
        cat(paste0("### ", .x, "\n\n"))
        cat("::::: {.panel-tabset}\n\n")
        tab.zone <- tab.type %>% filter(Zones == (.x != "Whole Harbour"))
        walk(unique(tab.zone$include_lags), ~{
            cat(paste0("#### ", .x, "\n\n"))
            tab.zone %>% filter(include_lags == .x) %>%
                `[`(, 'Kable') %>% `[[`(1) %>% `[[`(1) %>%
                print()
        })
        cat("::::: \n\n")
...
for (i in formatted.table.R2 %>% pull(Type) %>% unique()) {
    ## Type.name <- (formatted.table.R2 %>% pull(Type) %>% unique())[i]
    x <- formatted.table.R2 %>% filter(Type == i)
    x.c <- formatted.table.Corr %>% filter(Type == i)
    cat(paste0("## ", i, "\n"))
    cat(":::: {.panel-tabset}\n\n")
    for (j in c("Whole Harbour", "Zones")) {
        xx <- x %>% filter(Zones==(j != "Whole Harbour"))
        xx.c <- x.c %>% filter(Zones==(j != "Whole Harbour"))
        cat(paste0("### ", j, "\n"))
        cat("::::: {.panel-tabset}\n\n")
        for (k in xx %>% pull(include_lags) %>% unique()) {
            xxx.R2 <- (xx %>% filter(include_lags == k)) %>% `[`("data") %>%
                `[`("data") %>% `[[`(1) %>% `[[`(1) %>%
                arrange(ZoneName)
            xxx.Corr <- (xx.c %>% filter(include_lags == k)) %>% `[`("data") %>%
                `[`("data") %>% `[[`(1) %>% `[[`(1) %>%
                arrange(ZoneName)
            ## xxx.Corr <- (formatted.table.Corr %>% filter(Type == i,
            ##                                            Zones == (j != "Whole Habour"),
            ##                                            include_lags ==k)) %>%
                ## `[`("data") %>% `[[`(1) %>% `[[`(1)
            cols <- colnames(xxx.Corr)[-1:-2]
            cat(paste0("#### ", ifelse(k==TRUE, 'Include lags', 'No lags'), "\n"))
            X <- xxx.R2 %>%
                mutate(TableLabel = str_replace(TableLabel, '\\$', '')) %>%
                rename(Pressure = TableLabel) %>%
                #arrange(ZoneName) %>% 
                kableExtra::kbl(align = c('l','l','c','c','c','c','c','c'),
                                escape = FALSE,
                                caption = 'Cross tabulation of partial R² values for each Pressure and Stressor pair.  Cell colours represent correlative polarity (positive: green, negative: red) and absolute magnitude (high\
er values equate to darker shades).') %>%
    purrr::reduce(
               which(names(xxx.R2) %in% cols),
               function(x, y) {
                   col <- xxx.Corr[,y]
    ...
    row_spec(seq(0, nrow(xxx.Corr), 1), extra_css = "border: 1px solid black;") %>%
    ## row_spec(0, align = 'center') %>% 
    ##{ if (length(unique(xxx.R2$ZoneName))>1) pack_rows('',index = table(xxx.R2$ZoneName)) else . } %>%
    kableExtra::kable_styling(font_size = 12)
            
            if (length(unique(xxx.R2$ZoneName))>1) {
                X %>% pack_rows('',index = table(xxx.R2$ZoneName)) %>%
                print() %>%
                    suppressMessages() %>%
                    suppressWarnings()
            } else {
...
##                     escape = FALSE,
##                     caption = 'Cross tabulation of partial R2 values for each Pressure and Stressor pair.  Cell colours represent correlative polarity (positive: green, negative: red) and absolute magnitude (high\
## er values equate to darker shades).') %>%
##     purrr::reduce(
